...
class DPDataset(Dataset, ABC):
    ...
    def _featurize(self, save=True):
        ...
        self.featurized_drugs = [self.drug_featurizer(drug) for drug in tqdm(self.drugs)] # 原来是这样
    ...
    def _configure_featurizer(self):
        if len(self.config["mol"]["modality"]) > 1:
            ...
            self.drug_featurizer.set_mol2text_dict(self.smi2text) # TODO: self.mol2text is from load_data
        else:
...
class MoleculeNetDataset(DPDataset):
    ...
    name2text = {
      "bbbp": "The blood-brain barrier (BBB) is an important defence mechanism that restricts disease-causing pathogens and toxins to enter the brain from the bloodstream. The blood-brain barrier penetration (BBBP) dataset is designed for the modeling and prediction of barrier permeability. As a membrane separating circulating blood and brain extracellular fluid, the blood-brain barrier blocks most drugs, hormones, and neurotransmitters. Thus penetration of the barrier forms a long-standing issue in the development of drugs targeting the central nervous system.",
      "clintox": "Clinical trials are prospective biomedical or behavioral research studies on human participants designed to answer specific questions about biomedical or behavioral interventions, including new treatments (such as novel vaccines, drugs, dietary choices, dietary supplements, and medical devices) and known interventions that warrant further study and comparison. Clinical trials generate data on dosage, safety and efficacy. This is a task about qualitative data of drugs that failed clinical trials for toxicity reasons",
      "tox21": "Qualitative toxicity measurements including nuclear receptors and stress response pathways",
      "toxcast": "Compounds based on in vitro high-throughput screening",
      "sider": "Adverse drug reactions (ADRs) – unintended, harmful events attributed to the use of medicines – occur as a cause of and during a significant proportion of unscheduled hospital admissions.A careful medication history can assist a prescriber in understanding the patient's previous experiences with drug treatment, particularly in identifying previous ADRs that may preclude re-exposure to the drug. This is a task about marketed drugs and adverse drug reactions (ADR), grouped into 27 system organ classes",
      "hiv": "Experimentally measured abilities to inhibit HIV replication",
      "bace": "The binding affinities (IC50) reported for diverse structural and chemical classes of human β-secretase 1 (BACE-1) inhibitors in literature were modeled using multiple in silico ligand based modeling approaches and statistical techniques. The task provides quantitative (IC50) and qualitative (binary label) binding results for a set of inhibitors of human β-secretase 1 (BACE-1). All data are experimental values reported in scientific literature over the past decade, some with detailed crystal structures available. A collection of 1522 compounds with their 2D structures and properties are provided.",
      "muv": "Subset of PubChem BioAssay designed for validation of virtual screening techniques",
    }
    """
